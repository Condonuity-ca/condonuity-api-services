...
public interface ClientOrganisationRepository extends JpaRepository<ClientOrganisation, Integer> {
	...
	@Modifying
	@Transactional
	@Query(value="UPDATE client_organisation SET organisation_name = (?1) , corporate_number = (?2) WHERE client_organisation_id=(?3);", nativeQuery = true)
...
//	@Query("SELECT co.* FROM condonuitydev.client_organisation co where co.client_organisation_id = (?1) and ( co.organisation_name or co.management_company ) LIKE '%(?2)%';")
	...
	@Query(value="Select co.* FROM client_organisation co where ( co.delete_status = 1 or co.delete_status = 2) and ( co.active_status = 1 );", nativeQuery = true)
	List<ClientOrganisation> findAllActiveInActiveOrganisations();

	@Query(value="Select co.* FROM client_organisation co where ( co.organisation_name LIKE (?1) and ( co.delete_status = 1 or co.delete_status = 2) and ( co.active_status = 1 ));", nativeQuery = true)
	List<ClientOrganisation> findAllActiveInActiveOrganisationsByOrganisationName(String keyword);

	@Query(value="Select co.* FROM client_organisation co where ( co.corporate_number LIKE (?1) and ( co.delete_status = 1 or co.delete_status = 2) and ( co.active_status = 1 ));", nativeQuery = true)
	List<ClientOrganisation> findAllActiveInActiveOrganisationsByCorporationNumber(String keyword);
	
	@Query(value="Select co.* FROM client_organisation co where ( co.organisation_name LIKE (?1) and ( co.active_status = 0 or co.active_status = 2 ));", nativeQuery = true)
	List<ClientOrganisation> findAllUnApproveRejectOrganisationsByOrganisationName(String keyword);

	@Query(value="Select co.* FROM client_organisation co where ( co.corporate_number LIKE (?1) and ( co.active_status = 0 or co.active_status = 2 ));", nativeQuery = true)
